...
// reflection).
func startGRPC(cfg *config.Config, logger *slogx.Logger, svc *logicv1.ShippingService) *grpc.Server {
	ctx := context.Background()
	lc := net.ListenConfig{}
...
func setupServer(cfg *config.Config, otelServiceName string, logger *slogx.Logger, isShuttingDown *atomic.Bool, handler *webv1.Handler, pool interface {
...
func runGracefulShutdown(
	...
	ctx := context.Background()
	go func() {
		logger.Info(ctx, "Starting shipping service", slog.String("port", cfg.Service.Port))
