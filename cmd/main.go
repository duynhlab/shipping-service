...
func main() {
	...
	// scraped /metrics endpoint — the flag-off status quo; must run before
	// grpcx.NewServer). When OTEL_METRICS_ENABLED=true, SetupObservability
	// below installs the OTLP MeterProvider as the global AFTER this,
	// deliberately superseding the bridge (RFC-0014 dual-emit: client_golang
	// scrape stays untouched either way; only the OTel-instrumented metrics
	// switch transport).
